  ...
  media_ingress_container_definitions = jsonencode([
    {
      ...
      command = [
        "sh",
        "-lc",
        "if [ -z \"$ANNOUNCED_IP\" ]; then ANNOUNCED_IP=$(node -e \"const https=require('https');https.get('https://checkip.amazonaws.com',r=>{let d='';r.on('data',c=>d+=c);r.on('end',()=>process.stdout.write(d.trim()));}).on('error',()=>process.exit(1));\"); export ANNOUNCED_IP; fi; exec npm run ingress",
      ]
      environment = [
  ...
  media_egress_container_definitions = jsonencode([
    {
      ...
      command = [
        "sh",
        "-lc",
        "if [ -z \"$ANNOUNCED_IP\" ]; then ANNOUNCED_IP=$(node -e \"const https=require('https');https.get('https://checkip.amazonaws.com',r=>{let d='';r.on('data',c=>d+=c);r.on('end',()=>process.stdout.write(d.trim()));}).on('error',()=>process.exit(1));\"); export ANNOUNCED_IP; fi; exec npm run egress",
      ]
      environment = [
        { name = "SIGNALING_HOST", value = var.signaling_host },
        { name = "SIGNALING_PORT", value = "1188" },
