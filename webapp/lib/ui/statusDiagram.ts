  ...
  update(data: StatusData) {
    ...
    routerDumps.forEach((dump) => {
      ...
      routers.forEach((router) => {
        ...
        setAttr(node, "fill", "#fff7d6");
        setAttr(node, "stroke", "#b45309");
    ...
    pipes.forEach((pipe) => {
      ...
      attachTooltip(pipeLine, tooltipLines.join("\n"));
      ...
      const sendRate = ingressStats.rtpSendBitrate;
      const recvRate = ingressStats.rtpRecvBitrate;
      const sendRatePeer = egressStats.rtpSendBitrate;
      const recvRatePeer = egressStats.rtpRecvBitrate;
      if (
        typeof sendRate !== "number" ||
        typeof recvRate !== "number" ||
        typeof sendRatePeer !== "number" ||
        typeof recvRatePeer !== "number"
      ) {
        return;
      }
      const totalRate = sendRate + recvRate + sendRatePeer + recvRatePeer;
      const ingressSent = ingressStats.rtpBytesSent;
      const ingressRecv = ingressStats.rtpBytesReceived;
      const egressSent = egressStats.rtpBytesSent;
      const egressRecv = egressStats.rtpBytesReceived;
      if (
        typeof ingressSent !== "number" ||
        typeof ingressRecv !== "number" ||
        typeof egressSent !== "number" ||
        typeof egressRecv !== "number"
      ) {
        return;
      }
      const totalBytes = ingressSent + ingressRecv + egressSent + egressRecv;
      const statsLabel = makeText(
        `${formatRate(totalRate)} · ${formatBytes(totalBytes)}`,
        0,
        0,
        lineColor,
        8,
    ...
    peers.forEach((peer, index) => {
      ...
      if (roomId) {
        Object.entries(transportEgress).forEach(([egress, transportId]) => {
          ...
          const rate = stats.rtpSendBitrate;
          const bytes = stats.bytesSent;
          if (typeof rate !== "number" || typeof bytes !== "number") {
            return;
          }
      ...
      if (roomId) {
        Object.entries(transportIngress).forEach(([ingress, transportId]) => {
          ...
          const rate = stats.rtpRecvBitrate;
